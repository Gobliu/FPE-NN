...
t_gap = config.T_GAP
sigma = config.SIGMA
learning_rate_gh = config.LEARNING_RATE_GH * 0.01
learning_rate_p = config.LEARNING_RATE_P * 0.01
...
def main(run_id, p_patience, smooth_gh=0.1, smooth_p=False, sample_range=None):
    run_ = 0
    while run_ < 100:
        ...
        #                                                                            recur_win_p, run_)
        directory = '/home/liuwei/GitHub/Result/OU/id{}_p{}_win{}{}_{}_3in1'.format(run_id, p_patience, recur_win_gh,
                                                                                    recur_win_p, run_)
    ...
    pgh_3in1 = fpe_net_ng.recur_train_3in1(learning_rate=learning_rate_p, loss=Loss.sum_square_2in1)
    ...
    win_x, win_t, win_y, _ = PxtData_NG.get_recur_win_center(smooth_data.train_data, smooth_data.train_t,
                                                             recur_win_gh)
    ...
    for iter_ in range(n_iter):
        ...
        # train p
        pgh_3in1.get_layer(name=name + 'g').set_weights([gg_v_ng])
        pgh_3in1.get_layer(name=name + 'h').set_weights([hh_v_ng])
        ...
        for sample in range(n_sample):
            ...
            pgh_3in1.get_layer(name=name + 'p').set_weights([train_p_p_ng[sample].reshape(-1, 1, 1)])
        ...
        gg_v_ng = pgh_3in1.get_layer(name=name + 'g').get_weights()[0]
        hh_v_ng = pgh_3in1.get_layer(name=name + 'h').get_weights()[0]
...
if __name__ == '__main__':
    main(run_id=config.RUN_ID, p_patience=10, smooth_gh=0.1, smooth_p=True, sample_range=None)
