...
data = np.load('./Pxt/Boltz_id{}_{}_sigma{}.npz'.format(1, 821215, 0.01))
...
trained_data = np.load('/home/liuwei/OldDisk/GitHub/Result/Boltz/id1_p10_win77_0/iter248.npz')
trained_pxt = trained_data['P']

print(true_pxt.shape, noisy_pxt.shape, trained_pxt.shape)

for i in range(50, 100):
    print(i)
    plt.plot()
    # plt.text(-0.1, 1.10, 'A', fontsize=20, transform=ax.transAxes, fontweight='bold', va='top')
    plt.plot(x, true_pxt[i, 40, :], 'k', linewidth=1, label='clean P')
    # plt.scatter(x, noisy_pxt[i, 20, :], c='b', marker='+', s=40, label='Ep 0.01')
    plt.plot(x, noisy_pxt[i, 40, :], 'b', linewidth=1, label='noisy P')
    plt.plot(x, trained_pxt[i, 40, :], 'r', linewidth=1, label='trained P')
    plt.tick_params(direction='in', width=3, length=6)
    plt.ticklabel_format(axis="y", style="sci", scilimits=(0, 0))
    # plt.yticks(np.arange(0.005, 0.018, 0.003))
