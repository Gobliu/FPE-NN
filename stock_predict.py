...
def test_process(stock, dif, train_win, test_win):
    ...
    for idx in range(train_win-1, stock.shape[0]-test_win-1):
        ...
        if np.sum(dif[idx-train_win+1:idx+test_win+1]) > 0:
            ...
            test_y.append(stock[idx+1: idx+test_win+1, :])          # no transpose, unlike train_y
    ...
    for iter_ in [291]:
        ...
        if not os.path.exists(npz_path):
            ...
            for sample in range(n_sample):
                ...
                # test_p_y = np.copy(test_y[sample:sample+1])
                ...
                plt.figure()
                plt.plot(pred_p_after_train[0, :, -1], 'k-', label='after train', linewidth=1)
                plt.plot(trained_p, 'r', label='trained_p', linewidth=1)
...
if __name__ == '__main__':
    main(stock_type='FTSE', start=0, end=600, run_=2, train_win=5, test_win=5)
